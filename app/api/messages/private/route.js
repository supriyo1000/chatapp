...
export async function GET(request) {
    try {
        ...
        // console.log(fromUserId, toUserId, currentMsgId);
        // console.log(typeof(fromUserId),typeof(toUserId),typeof(currentMsgId));
        
        

        // Ensure the parameters are valid
        if (!fromUserId || !toUserId || !currentMsgId) {
        ...
        const [result] = await dbconnection.execute(
            'CALL message_master(?, ?, ?, ?, ?, ?)',
            [3, parseInt(fromUserId), parseInt(toUserId), '', null, currentMsgId === 'null' ? null : currentMsgId === null ? null :currentMsgId]  // Use actual values from request
        );
